...
print(f"RMSD FF1 vs before trends = {rmsd_ff1:.3f} µmol / (kg * yr)")
rmsd_ff2 = np.sqrt(np.mean((tra_ff2_sym.slope - xo.trb.slope) ** 2))
print(f"RMSD FF2 vs before trends = {rmsd_ff2:.3f} µmol / (kg * yr)")
lr_c = linregress(xo.trends_true, xo.trb.slope)
print(f"Panel (c) r-square ('before') = {lr_c.rvalue**2:.2f}")
rmsd_c_before = np.sqrt(np.mean((xo.trends_true - xo.trb.slope) ** 2))
print(f"RMSD before vs assigned = {rmsd_c_before:.3f} µmol / (kg * yr)")
rmsd_c_after = np.sqrt(np.mean((xo.trends_true - tra_sym.slope) ** 2))
print(f"RMSD after vs assigned = {rmsd_c_after:.3f} µmol / (kg * yr)")
